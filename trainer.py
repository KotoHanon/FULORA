...
from code.model.environment import ClusterEpisode
from code.model.environment import EntityEpisode
...
class Trainer(object):
	def __init__(self, params):
		...
		#self.device = 'cpu'
		#self.device = torch.device(torch.cuda.is_available() if 'cuda' else 'cpu')
		print(self.device)
		self.delta = 0.2
		self.e_agent = EntityAgent(params).to(self.device) # DWARF
		self.c_agent = ClusterAgent(params).to(self.device) # GIANT
	...
	def calc_cum_discounted_reward_credit(self, approx_credits, entity_rewards, cluster_rewards):
		...
		cum_disc_reward = torch.zeros([num_instances, self.path_length]).to(self.device)  # [original batch_size * num_rollout, T]
		cum_disc_reward[:,self.path_length - 1] = entity_rewards  # set the last time step to the reward received at the last state
	...
	def regularization_cluster(self, cluster_scores, e_agent_pred_clusters):
		...
		reg_loss = self.delta * torch.nn.functional.cross_entropy(cluster_scores, e_agent_pred_clusters)
		return reg_loss
	...
	def cluster_entity_embeddings_sim_reg(self):
		...
		for cls, ents in self.cluster2ent_.items():
			...
			ent_emb = torch.cat(ent_emb, dim=0) # origin dimension * 2
	...
	def train(self):
		...
		for entity_episode, cluster_episode in self.train_environment.get_episodes(self.batch_counter):
			...
			entity_rewards, entity_default_rewards = entity_episode.get_reward()
			...
			KL_loss = self.cluster_entity_embeddings_sim_reg()
			...
			# print statistics
			# train_loss = 0.98 * train_loss + 0.02 * reinforce_loss + 0.01 * KL_loss
			...
			reward_reshape = np.reshape(entity_default_rewards,
			...
			logger.info("Agents: batch_counter: {0:4d}, num_hits: {1:7.4f}, avg. reward per batch {2:7.4f}, "
						"num_ep_correct {3:4d}, avg_ep_correct {4:7.4f}, avg_clr_emb {5:7.4f}, avg_ent_emb {5:7.4f}, train loss {6:7.4f}".
						format(self.batch_counter, np.sum(entity_rewards), e_avg_reward + c_avg_reward, num_ep_correct,
								   (num_ep_correct / self.batch_size), ClusterEpisode.get_avg_clr_emb(cluster_episode),
							   EntityEpisode.get_avg_ent_emb(entity_episode), train_loss))
			wandb.log({'avg_clr_emb':ClusterEpisode.get_avg_clr_emb(cluster_episode)})
			wandb.log({'avg_ent_emb':EntityEpisode.get_avg_ent_emb(entity_episode)})
	...
	def test(self, beam=False, print_paths=False, save_model=True, mrr=False):
		...
		with torch.no_grad():
			...
			for entity_episode, cluster_episode in tqdm(self.test_environment.get_episodes(0)):
				...
				rewards, default_rewards = entity_episode.get_reward()
				reward_reshape = np.reshape(default_rewards,
			...
			with open(self.output_dir + '/scores.txt', 'a') as score_file:
				...
				score_file.write("\n")
				score_file.write("MRR: {0:7.4f}".format(mrr))
				score_file.write("\n")
				score_file.write("\n")
			...
			logger.info("MRR: {0:7.4f}".format(mrr))

		wandb.log({'Hits@1': all_final_reward_1, 'Hits@3': all_final_reward_3, 'Hits@5': all_final_reward_5,
				   'Hits@10': all_final_reward_10, 'Hits@20': all_final_reward_20, 'MRR': mrr})
